...
test('Analysis.res does not import Export.res', () => {
  const source = fs.readFileSync('src-/Analysis.res', 'utf-8');
...
- Analysis modules: `src-/Analysis.res`, `src-/Mapper.res` (hotspots, networks)
- Export modules: `src-/Export.res`, `src-/Visualization.res`
