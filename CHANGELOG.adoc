...
- `security/audit.sh` - Automated security scanner (, Rust, Trivy)
- `.trivyignore` - Security scanner configuration
- Multi-layer scanning:  cache, cargo audit, Trivy filesystem
...
- , , and WASM APIs documented
- Code examples for all major functions
- Auto-generation support via ` doc`
...
-  runtime replacing Node.js/npm (zero `npm install` needed)
-  for type-safe functional business logic
- Rust/WASM for performance-critical operations (compiled AOT)
-  as integration glue layer
...
-  compile-time type checking (`src-/UbiCity.res`)
-  integration layer (`src/**/*.ts`)
...
- Locked dependencies ( cache)
- CI/CD verification on every commit
- Docker support for containerized builds

**Developer Experience**
- `.json` replaces `package.json`
- `just` commands for all workflows
- Automatic code formatting (` fmt`)
- Built-in linter (` lint`)
- Integrated test runner (` test`)

=== Changed

- **BREAKING**: Migrated from Node.js to  runtime
- **BREAKING**: Core logic rewritten in  (functional paradigm)
- **BREAKING**: Performance-critical code moved to Rust/WASM
- **BREAKING**: Build system changed from npm to just +  tasks
- **BREAKING**: Module resolution via  import maps (not node_modules)
...
[source,bash]
----
= Install  (replaces Node.js)
curl -fsSL https:///x/install/install.sh | sh
...
**Missing for Silver Tier**: 80%+ test coverage (need to port v0.2 tests to )

=== Architecture

**Component Responsibilities**:
- ****: Domain model, business logic, pure functions
- ****:  integration, CLI, I/O operations
- **Rust/WASM**: Validation, graph algorithms, performance-critical paths
- ****: Runtime, permissions, standard library
...
- CLI-first ( tasks replace npm scripts)
- Offline-first (verified zero network calls)
- Privacy by default ( permissions enforce this)
- Constraint mechanisms (4-week experiment intact)

=== Technical Stack

**Runtime**:  1.x
**Languages**:  11.x, Rust 1.70+,  5.x
**Build Tools**: just, cargo, , wasm-opt
...
- Updated `QUICK_START.md` for  workflow

=== Removed

- `package.json` (replaced by `.json`)
- `package-lock.json` ( caches dependencies)
- `node_modules/` ( uses import maps)
- npm-specific scripts

=== Fixed

- Type safety gaps (now compile-time checked via )
...
**Testing**
- Port v0.2 tests to  test framework
- Measure test coverage (` coverage`)
