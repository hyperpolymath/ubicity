...
This guide helps you migrate from the original CommonJS codebase to the
modern ESM version.
...
*For Existing Users*: Your data is compatible. Update your scripts to
use ESM imports and new CLI.
...
*Action Required*: If you have custom scripts using UbiCity modules,
update them to use ESM imports.
...
*Action Required*: Add `+async+`/`+await+` to your code. Wrap in async
functions or use `+.then()+`.
...
*Action Required*: Update shell scripts and documentation to use new
paths.
...
The v0.2 JSON schema is a superset of v0.1. All existing experience
...
If you see errors, the new validation caught data quality issues. Use
the migration helper (see below).
...
Common fixes: - Add missing timestamps - Normalize coordinate formats -
Add version field - Validate against Zod schema

=== Feature Mapping

[width="100%",cols="38%,44%,18%",options="header",]
|===
...
|`+mapper.js learner <id>+` |`+src/cli.js learner <id>+` |Enhanced
journey view
...
*Week 1*: Install v0.2, verify data compatibility *Week 2*: Start using
new CLI for analysis *Week 3*: Update capture workflow (if you have
custom scripts) *Week 4*: Migrate custom analysis code to ESM *Week 5*:
Delete old CommonJS files (if confident)

=== Getting Help

*Issues*: Open a GitHub issue with "`migration`" tag *Questions*: Check
`+GETTING_STARTED.md+` for v0.2 usage *Rollback*: See "`Rollback Plan`"
section above
...
But remember: *You don’t need any of this to start capturing learning
experiences.*
...
*Questions? Something broken?* File an issue:
https://github.com/Hyperpolymath/ubicity/issues
