...
* link:#core-classes[Core Classes]
* link:++#schemas--validation++[Schemas & Validation]
...
*`+async initialize()+`* Initialize storage directories.
...
*`+async loadAll()+`* Load all experiences from storage.
...
*`+async captureExperience(data)+`* Capture a new learning experience.
...
*`+findInterdisciplinaryConnections()+`* Find experiences spanning
multiple domains.
...
*`+mapByLocation()+`* Aggregate experiences by location.
...
*`+getLearnerJourney(learnerId)+`* Get a learner’s journey over time.
...
*`+generateDomainNetwork()+`* Generate domain co-occurrence network.
...
*`+findLearningHotspots(minDiversity = 3)+`* Find locations with high
...
*`+async generateReport()+`* Generate comprehensive analysis report.
...
*`+generateId()+`* Generate unique ID (ubi-\{uuid}).

*`+validate()+`* Validate experience data (throws on error).
...
*`+safeValidate()+`* Safe validation (returns \{ success, errors }).
...
[arabic]
. *Initialize before use*: Always call `+await mapper.initialize()+` and
`+await mapper.loadAll()+`
. *Validate early*: Use `+safeValidateExperience()+` for user input
. *Handle errors*: Wrap async operations in try/catch
. *Use plugins for extensibility*: Don’t modify core code, create
plugins
. *Monitor performance*: Use `+PerformanceMonitor+` for bottlenecks
. *Respect privacy*: Always anonymize before sharing data
. *Export regularly*: Back up data using export utilities
...
* link:../GETTING_STARTED.md[GETTING_STARTED.md] - Quick start guide
* link:../MINIMAL_VIABLE_PROTOCOL.md[MINIMAL_VIABLE_PROTOCOL.md] - Core
philosophy
* link:../MIGRATION.md[MIGRATION.md] - Upgrade guide
