...
function decodeCoordinates(json) {
  var lat = getNumber(json, "latitude");
  var latResult;
  latResult = lat.TAG === "Ok" ? ({
        TAG: "Ok",
        _0: lat._0
      }) : getNumber(json, "lat");
  var lon = getNumber(json, "longitude");
  var lonResult;
  lonResult = lon.TAG === "Ok" ? ({
        TAG: "Ok",
        _0: lon._0
      }) : getNumber(json, "lon");
  if (latResult.TAG === "Ok") {
    if (lonResult.TAG === "Ok") {
      return {
              TAG: "Ok",
              _0: {
                latitude: latResult._0,
                longitude: lonResult._0
              }
            };
    } else {
      return {
              TAG: "Error",
              _0: "Coordinates decode error: " + lonResult._0
            };
    }
  } else {
    return {
            TAG: "Error",
            _0: "Coordinates decode error: " + latResult._0
          };
  }
}
...
function decodeLearningExperience(json) {
  ...
  var id = Core__Option.getOr(getOptionalString(json, "id"), "ubi-" + $$Crypto.randomUUID());
  var timestamp = Core__Option.getOr(getOptionalString(json, "timestamp"), Date.now().toString());
  var version = Core__Option.getOr(getOptionalString(json, "version"), "1.0");
  var err;
