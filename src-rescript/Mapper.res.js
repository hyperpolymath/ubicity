...
import * as Decoder from "./Decoder.res.js";
...
function loadAll(mapper) {
  return Core__Promise.$$catch(mapper.storage.loadAllExperiences().then(function (experiencesData) {
                  var err = Decoder.decodeExperiences(experiencesData);
                  if (err.TAG !== "Ok") {
                    return Promise.resolve({
                                TAG: "Error",
                                _0: "Decode failed: " + err._0
                              });
                  }
                  var experiences = err._0;
                  experiences.forEach(function (exp) {
                        mapper.experiences[exp.id] = exp;
                      });
                  var updated = Core__Array.reduce(experiences, mapper, (function (m, exp) {
                          return updateIndices(m, exp);
                        }));
                  Object.entries(updated.indices.location).forEach(function (param) {
                        mapper.indices.location[param[0]] = param[1];
                      });
                  Object.entries(updated.indices.domain).forEach(function (param) {
                        mapper.indices.domain[param[0]] = param[1];
                      });
                  Object.entries(updated.indices.learner).forEach(function (param) {
                        mapper.indices.learner[param[0]] = param[1];
                      });
                  return Promise.resolve({
                              TAG: "Ok",
                              _0: experiences.length
                            });
                }), (function (param) {
                return Promise.resolve({
                            TAG: "Error",
